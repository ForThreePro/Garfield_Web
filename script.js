const monedas = {
  PEN: {symbol: 'S/', rate: 1},
  USD: {symbol: '$', rate: 0.267},
  CLP: {symbol: '$', rate: 250},
  ARS: {symbol: '$', rate: 400},
  MXN: {symbol: '$', rate: 5},
  BOB: {symbol: '$', rate: 3},
  UYU: {symbol: '$', rate: 10},
  COP: {symbol: '$', rate: 1000}
};
...
function updateCart(){
  ...
  cart.forEach((i, idx) => {
    total += i.price;
    items.innerHTML += `<div style="display:flex; justify-content:space-between">${i.name} - ${monedas[currentCurrency].symbol}${(i.price * monedas[currentCurrency].rate).toFixed(2)} <button onclick="removeItem(${idx})">x</button></div>`;
  });
